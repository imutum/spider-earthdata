...
logging.basicConfig(level=logging.ERROR, format='%(name)s %(asctime)s %(levelname)s: %(message)s', datefmt="%Y-%m-%d %H:%M:%S")
_logger = logging.getLogger(os.path.splitext(os.path.basename(__file__))[0])
_logger.setLevel(logging.DEBUG)


def _is_web_file_from_url(url):
    last_text = url.split("/")[-1]  # 获取URL里最后一节字符串，用于判断是文件还是目录
    if "." in last_text:  # 判断是url是否是文件夹链接
        return True
    else:
        return False
...
def _get_info_from_url(url, method=None):
    _logger.info(url)
    if _is_web_file_from_url(url):  # 判断是url是否是文件夹链接
        return
    if method is None:
        method = requests.get
    while True:
...
def loop_info(urls:list, thread_num=20, method=None):
    ...
    df_flag_isfile = df.loc[:, "name"].apply(_is_web_file_from_url)
